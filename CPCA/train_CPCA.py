    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        with torch.no_grad():
            ...
            for batch, (moving, fixed) in enumerate(val_loader):
                ...
                loss_Jacobian = criterions[1](output[1], y).cpu()
                # loss_Jacobian = jacobian_determinant(output[1][0])
                ncc_loss_ori = image_loss_func_NCC(output[0], y).cpu()
                mse_loss = MSE(output[0], y).cpu()
                loss_sum = (ncc_loss_ori + weights[1] * loss_Jacobian).cpu()
                losses.append([ncc_loss_ori.item(), mse_loss.item(), loss_Jacobian, loss_sum.item()])
