...
from utilses.utilize import plotorsave_ct_scan, get_project_path, make_dir
...
def crop_resampling_resize_clamp(sitk_img, new_size=None, crop_range=None, spacing=None, clamp=None):
    ...
    """
    # crop
    if crop_range is not None:#特定区域裁剪图像
...
def resample_image(itk_image, restore=False, out_spacing=[1.0, 1.0, 1.0], is_label=False):
    ...
    if restore:
        ...
        img_tensor = F.interpolate(img_tensor.unsqueeze(0).unsqueeze(0), size=[96, 144, 144], mode='trilinear', align_corners=True)
    ...
    for num_fixed, file_name_fixed in enumerate(os.listdir(file_folder)):
        ...
        if 'T05' in file_name_fixed:
            for num_moving, file_name_moving in enumerate(os.listdir(file_folder)):
                ...
                shutil.copyfile(os.path.join(file_folder, file_name_fixed), fixed_file_path)#复制文件(源->目标)
                shutil.copyfile(os.path.join(file_folder, file_name_moving), moving_file_path)#复制文件(源->目标)
...
def dirlab_test(args, file_folder, m_path, f_path, datatype, shape, case):
    for file_name in os.listdir(file_folder):
        ...
        file_path = os.path.join(file_folder, file_name)
        file = np.memmap(file_path, dtype=datatype, mode='r')#创建一个内存映射对象
        if shape:
            file = file.reshape(shape)

        img = sitk.GetImageFromArray(file)#将 NumPy 数组转换为 ITK 图像对象
        ...
        target_file_path = os.path.join(target_path,case_name)
...
if __name__ == '__main__':
    ...
    # dirlab train
    data_path = r'D:\datasets\datasets_raya\dirlab_train_144_144_96'
    make_dir(data_path)
    for item in dirlab_case_cfg.items():
        ...
        make_dir(save_path)
        img_path = f'D:/datasets/dirlab/img/Case{case}Pack/Images'
    ...
    # # dirlab for test
    # print("dirlab: ")
    # target_test_fixed_path = r'D:\datasets\datasets_raya\fixed'
    # target_test_moving_path = r'D:\datasets\datasets_raya\moving'
    # make_dir(target_test_moving_path)
    # make_dir(target_test_fixed_path)
    # for item in dirlab_case_cfg.items():
    #     case = item[0]
