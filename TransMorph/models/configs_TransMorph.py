...
def get3DTranUnet_config():
    """
    get3DTranUnet
    """
    config = ml_collections.ConfigDict()
    config.hidden_dim = 256
    config.inputsize = (40, 40, 40)
    config.patchsize = (4, 4, 4)
    config.trans_layers = 2
    config.embedding_dprate = 0.1
    config.head_num = 4
    config.emb_in_channels = 32
    config.mlp_dprate = 0.2
    config.n_class = 2
    config.last_in_ch = 16
    config.n_skip = 3
    config.head_channels = 128
    config.decoder_channels = [(128, 64, 32), (64, 32, 16), (32, 16, 8)]
    return config
