...
class chang_dual_WindowAttention(nn.Module):
    """ Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    Args:
        dim (int): Number of input channels.
    ...
    def forward(self, x, mask=None):
        ...
        if (B_ >0 and B_ % 2 == 0):
            B_A = B_ // 2
            cross_x1 = torch.zeros((B_A, N, C), device=x.device)
            cross_x2 = torch.zeros((B_A, N, C), device=x.device)
            cross_x1[:, :, :] = x[0:B_A, :, :]
            cross_x2[:, :, :] = x[B_A:, :, :]

            qkv1 = self.qkv(cross_x1).reshape(B_//2, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            qkv2 = self.qkv(cross_x2).reshape(B_//2, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)

        elif(N > 0  and N % 2 == 0 ):
            N_A = N // 2
            cross_x1 = torch.zeros((B_, N_A, C), device=x.device)
            cross_x2 = torch.zeros((B_, N_A, C), device=x.device)
            cross_x1[:, :, :] = x[:, 0:N_A, :]
            cross_x2[:, :, :] = x[:, N_A:, :]
            qkv1 = self.qkv(cross_x1).reshape(B_, N//2, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            qkv2 = self.qkv(cross_x2).reshape(B_, N//2, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)

        elif (C > 0  and C % 2 == 0):
            C_A = C // 2
            cross_x1 = torch.zeros((B_, N, C_A), device=x.device)
            cross_x2 = torch.zeros((B_, N, C_A), device=x.device)
            cross_x1[:, :, :] = x[:, :, 0:C_A]
            cross_x2[:, :, :] = x[:, :, C_A:]
            qkv1 = self.qkv(cross_x1).reshape(B_, N, 3, self.num_heads, C_A // self.num_heads).permute(2, 0, 3, 1, 4)
            qkv2 = self.qkv(cross_x2).reshape(B_, N, 3, self.num_heads, C_A // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        # qkv1 = self.qkv(cross_x1).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # qkv2 = self.qkv(cross_x2 ).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        b1,b2,b3 = cross_x1.shape

        # x1 = (attn1 @  v1).transpose(1, 2).reshape(B_, N, C)
        x1 = (attn1 @ v1).transpose(1, 2).reshape(b1, b2, b3)
        ...
        x2 = (attn2 @ v2).transpose(1, 2).reshape(b1, b2, b3)
...
class cross_WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # if (B_ >0 and B_ % 2 == 0):
        cross_x1 = torch.zeros((B_, N, C//2), device=x.device)
        cross_x2 = torch.zeros((B_, N, C//2), device=x.device)
        cross_x1[:, :, :] = x[:, :, :C//2]
        cross_x2[:, :, :] = x[:, :, C//2:]

        qkv1 = self.qkv(cross_x1).reshape(B_, N, 3, self.num_heads, C//2 //self.num_heads).permute(2, 0, 3, 1, 4)
        q1, k1, v1 = qkv1[0], qkv1[1], qkv1[2]  # make torchscript happy (cannot use tensor as tuple)
        qkv2 = self.qkv(cross_x2 ).reshape(B_, N, 3, self.num_heads, C//2 //self.num_heads).permute(2, 0, 3, 1, 4)
        q2, k2, v2 = qkv2[0], qkv2[1], qkv2[2]  # make torchscript happy (cannot use tensor as tuple)
        q1 = q1 * self.scale
        q2 = q2 * self.scale
        #attention
        ...
        x1 = (attn1 @  v2).transpose(1, 2).reshape(B_, N, C//2)
        ...
        x2 = (attn2 @ v1).transpose(1, 2).reshape(B_, N, C//2)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        self.chang_dual_attn = chang_dual_WindowAttention(
            dim, window_size=self.window_size, num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, rpe=rpe, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, mask_matrix):
        ...
        shortcut = x  #torch.Size([1, 64000, 96])
        x = self.norm1(x)
        x = x.view(B, H, W, T, C)
        ...
        # else :
            # # W-MSA/SW-MSA
        # attn_windows = self.chang_dual_attn(x_windows, mask=attn_mask)  # nW*B, window_size*window_size*window_size, C
            # else:
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, T):
        ...
        """
        #这段代码的目的是根据给定的窗口大小和移动步长，为图像生成一个掩码（img_mask）来标记每个窗口的索引。
        ...
        mask_windows = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, 1  torch.Size([336, 5, 6, 7, 1])
        mask_windows = mask_windows.view(-1, self.window_size[0] * self.window_size[1] * self.window_size[2])#torch.Size([336, 210])
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        patch_size = to_3tuple(patch_size)  # 将patch_size转换为3元组，并将其赋值给self.patch_size
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B C Wh Ww Wt b  torch.Size([1, 96, 40, 40, 40])
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
                 ...
                 in_chans=3,  # Number of input image channels. Default: 3
                 embed_dim=96,  # Patch embedding dimension. Default: 96
                 depths=[2, 2, 6, 2],  # Depth of each Swin Transformer layer.
                 num_heads=[3, 6, 12, 24],  # Number of attention heads in different layers.
                 window_size=(7, 7, 7),  # Window size. Default: 7
                 ...
                 qk_scale=None,
                 # Override default qk scale of head_dim ** -0.5 if set. Default: None则覆盖默认的qk缩放因子，qk缩放因子默认为head_dim ** -0.5
                 drop_rate=0.,  # Dropout rate. Default: 0
                 attn_drop_rate=0.,  # Attention dropout rate. Default: 0
                 drop_path_rate=0.2,  # Stochastic depth rate. Default: 0.1 随机深度的概率，默认0，1
                 ...
                 frozen_stages=-1,  #
                 use_checkpoint=False,  # Whether to use checkpointing to save memory. Default: False
                 pat_merg_rf=2,MC_drop=0.15 ):  #
        super().__init__()
        ...
        if self.ape:
            ...
            patches_resolution = [pretrain_img_size[0] // patch_size[0], pretrain_img_size[1] // patch_size[1],
                                  pretrain_img_size[2] // patch_size[2]]  # 表示图像被分解为多少个大小为patch_size的块。
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        # x = self.up(x)
        x = nn.functional.interpolate(x, skip.shape[2:], mode='trilinear', align_corners=True)
        if skip is not None:
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        self.resblock_group = self.resblock_seq(embed_dim * 8, bias_opt=False)
        self.resblock_group1 = self.resblock_seq(embed_dim * 4, bias_opt=False)
        self.resblock_group2 = self.resblock_seq(embed_dim * 2, bias_opt=False)
        self.resblock_group3 = self.resblock_seq(embed_dim , bias_opt=False)
    ...
    def forward(self, x, infer=True):
        ...
        if self.if_convskip:
            x_s0 = x.clone()
            x_s1 = self.avg_pool(x)#torch.Size([1, 2, 80, 80, 80])
            f4 = self.c1(x_s1)#torch.Size([1, 48, 80, 80, 80])
            f4 = nnf.dropout3d(f4, self.mc_drop, training=True)
            f5 = self.c2(x_s0)#torch.Size([1, 16, 160, 160, 160])
            f5 = nnf.dropout3d(f5, self.mc_drop, training=True)
        else:
            f4 = None
            f5 = None
        #x(b,c,d,h,w)->x(b,c,h,w,d)
        out_feats = self.transformer(x.permute(0,1,3,4,2))
        out_feats[-1] =self.resblock_group (out_feats[-1])
        if self.if_transskip:
            f1 = out_feats[-2]#torch.Size([1, 384, 10, 10, 10])
            f2 = out_feats[-3]#torch.Size([1, 192, 20, 20, 20])
            f3 = out_feats[-4]#torch.Size([1, 96, 40, 40, 40])
        ...
        f1 = self.resblock_group1(f1)
        # f2 = self.resblock_group2(f2)
        # f3 = self.resblock_group3(f3)

        x = self.up0(out_feats[-1], f1)#torch.Size([1, 384, 10, 10, 10])
        x = self.up1(x, f2)#torch.Size([1, 192, 20, 20, 20])
        x = self.up2(x, f3)#torch.Size([1, 96, 40, 40, 40])
        x = self.up3(x, f4)#torch.Size([1, 48, 80, 80, 80])
        x = self.up4(x, f5)#torch.Size([1, 16, 160, 160, 160])
        flow = self.reg_head(x)#torch.Size([1, 3, 160, 160, 160])
