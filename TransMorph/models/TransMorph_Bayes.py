...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        _, _, H, W, T = x.size()
        #这段代码是对输入的图像进行 padding 操作，确保图像的时间（T）维度是 patch_size 的整数倍。
        ...
        if self.norm is not None:
            ...
            x = x.flatten(2).transpose(1, 2)#(B, C, THW)=> (B, THW, C)
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.embed_dim, Wh, Ww, Wt)#embed_dim=96
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        """Forward function."""
        #slipt into patches
        x = self.patch_embed(x)#x = x.transpose(1, 2).view(-1, self.embed_dim, Wh, Ww, Wt)#embed_dim=96
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        # x = self.up(x)#x=[1,192,20,20,20]
        x = nn.functional.interpolate(x, skip.shape[2:], mode='trilinear', align_corners=True)#这个地方修改x = self.up(x)并且添加这条代码torch.Size([1, 192, 40, 40, 40])
        if skip is not None:
            x = torch.cat([x, skip], dim=1)#torch.Size([1, 288, 40, 40, 40])
        x = self.conv1(x)#torch.Size([1, 96, 40, 40, 40])
        x = self.conv2(x)#torch.Size([1, 96, 40, 40, 40])
        return x
...
class TransMorph(nn.Module):
    ...
    def forward(self, x):
        if self.if_convskip:
            x_s0 = x.clone()#torch.Size([1, 2, 160, 160, 160])
            #deown sampling+conv3D
        ...
        if self.if_transskip:
            f1 = out[-2]#torch.Size([1, 384, 10, 10, 10])
            f2 = out[-3]#torch.Size([1, 192, 20, 20, 20])
            f3 = out[-4]#torch.Size([1, 96, 40, 40, 40])
        else:
            f1 = None
            f2 = None
            f3 = None
...
class TransMorphBayes(nn.Module):
    ...
    def forward(self, x,infer=True):
        source = x[:, 0:1, :, :]#[1,1,160,160,160]
        x = self.net(x)#torch.Size([1, 16, 160, 160, 160])
        flow = self.reg_head(x)#[1,3,160,160,160]
